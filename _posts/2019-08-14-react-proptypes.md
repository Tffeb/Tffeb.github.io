...
1. 首先需要通过在终端npm install prop-types或yarn add prop-types安装一个叫prop-types的第三方包

2. 然后通过下面的写法对某一个组件中的变量进行类型检测：
...
无报错，把 `<Son object={\{name:'彭湖湾',age:20\}} />` 改成 `<Son object={\{name:'彭湖湾',age:'20'\}} />` ，然后就能报错了
